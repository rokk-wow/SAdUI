...
function addon:Initialize()
    ...
    self:AddSettingsPanel("options", {
        title = "options",
        controls = {{
            type = "checkbox",
            name = "showBottomBackground",
            default = true,
            onValueChange = function(addonInstance, isChecked)
                if isChecked then
                    addonInstance.onPlayerEnteringWorld.CreateBottomBackground(addonInstance)
                else
                    if addonInstance.SAdUI_BottomBackground then
                        addonInstance.SAdUI_BottomBackground:Hide()
                    end
                end
            end
        }}
...
do
    function addon.onPlayerEnteringWorld.CreateBottomBackground(self)
        if not self:GetValue("options", "showBottomBackground") then
            return
        end

        if self.SAdUI_BottomBackground then
            self.SAdUI_BottomBackground:Show()
            return
        end

        local bottomBg = CreateFrame("Frame", nil, UIParent)
        bottomBg:SetHeight(188)
        bottomBg:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 0, 0)
        bottomBg:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", 0, 0)
        bottomBg:SetFrameStrata("BACKGROUND")
        bottomBg:SetFrameLevel(0)

        local bg = bottomBg:CreateTexture(nil, "BACKGROUND")
        bg:SetAllPoints(bottomBg)
        bg:SetColorTexture(0, 0, 0, 0.5)

        self.SAdUI_BottomBackground = bottomBg
    end
end
...
do
    function addon.onPlayerEnteringWorld.CustomizeClock(self)
        ...
        if clockButton then
            ...
            clockButton:SetPoint("TOP", UIParent, "TOP", 0, -30)
...
do
    function addon.onPlayerEnteringWorld.SetCVars(self)
        ...
        self:Retry(function()
            ...
            return BattlefieldMapFrame and BattlefieldMapFrame:IsVisible()
        end)
    end
    ...
    function addon.onPlayerEnteringWorld.CustomizeBattlefieldMap(self)
        ...
        if mapFrame then
            -- Set battlefield map to 100% opacity
            if BattlefieldMapOptions then
                BattlefieldMapOptions.opacity = 0 -- 0 = fully opaque (100%)
    ...
    function addon.onPlayerEnteringWorld.ScaleZoneMap(self)
        ...
        local mapFrame = BattlefieldMapFrame
        if mapFrame then
            -- Get the default height and calculate scale to achieve target height
            local defaultHeight = mapFrame:GetHeight()
            if defaultHeight > 0 then
                local scale = targetHeight / defaultHeight
                mapFrame:SetScale(scale)
            end

            -- Position the map at the lower right corner of the screen
            mapFrame:ClearAllPoints()
            mapFrame:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", 2, -3)

            -- Attach the map tab to the top left of the battlefield map
            if BattlefieldMapTab then
                BattlefieldMapTab:ClearAllPoints()
                BattlefieldMapTab:SetPoint("BOTTOMLEFT", mapFrame, "TOPLEFT", 0, 0)
            end
        end
    end
end
...
do
    function addon.onPlayerEnteringWorld.CustomizeMinimap(self)
        ...
        local minimapWidth = minimapSize
        local minimapHeight = minimapSize
        ...
        -- Try to unclamp any edit mode manager frame selection
...
do
    function addon.onPlayerEnteringWorld.UpdateSettings(self)
        ...
        -- SetCVar("UnitNameNPC", 0) -- NPC Names: quest, hostile, and interactive
        ...
        SetCVar("nameplateShowEnemyMinions", 0) -- Enemy minions: true
...
--         infoBox:SetSize(300, 187)
--         infoBox:SetPoint("LEFT", Minimap, "RIGHT", -2, 0)

--         local bg = infoBox:CreateTexture(nil, "BACKGROUND")
--         bg:SetAllPoints(infoBox)
--         bg:SetColorTexture(0, 0, 0, 0.6)

--         self:addBorder(infoBox)

--         -- Create text elements in 2 columns
--         local leftX = 30
--         local rightX = 180
--         local yOffset = -10

--         -- Get the saved font preference or use default
--         local selectedFont = "Interface/AddOns/SAdUI/Media/Fonts/FiraMonoMedium.ttf"

--         -- LEFT COLUMN
--         infoBox.ilvl = infoBox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
--         infoBox.ilvl:SetPoint("TOPLEFT", infoBox, "TOPLEFT", leftX, yOffset)
...
--         infoBox.gold:SetPoint("TOPLEFT", infoBox, "TOPLEFT", leftX, yOffset)
...
--         infoBox.conquest:SetPoint("TOPLEFT", infoBox, "TOPLEFT", leftX, yOffset)
...
--         infoBox.honor:SetPoint("TOPLEFT", infoBox, "TOPLEFT", leftX, yOffset)
...
--         infoBox.durability:SetPoint("TOPLEFT", infoBox, "TOPLEFT", leftX, yOffset)
...
--         infoBox.vers:SetPoint("TOPLEFT", infoBox, "TOPLEFT", leftX, yOffset)
...
--         infoBox.haste:SetPoint("TOPLEFT", infoBox, "TOPLEFT", rightX, yOffset)
...
--         infoBox.mastery:SetPoint("TOPLEFT", infoBox, "TOPLEFT", rightX, yOffset)
...
--         infoBox.speed:SetPoint("TOPLEFT", infoBox, "TOPLEFT", rightX, yOffset)
...
--         infoBox.fps:SetPoint("TOPLEFT", infoBox, "TOPLEFT", rightX, yOffset)
...
--         infoBox.ping:SetPoint("TOPLEFT", infoBox, "TOPLEFT", rightX, yOffset)
